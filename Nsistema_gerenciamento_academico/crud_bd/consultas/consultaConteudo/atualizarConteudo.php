...
if ($_SERVER["REQUEST_METHOD"] === "POST") {
    ...
    if ($stmt->rowCount() > 0) {
        ...
        header("Location: ../../consultas/consultaConteudo/consultaConteudo.php?message=" . urlencode($message));
        exit();
    } else {
        $error = "Erro ao atualizar o conteúdo com ID $id_conteudo.";
        header("Location: ../../cadastros/cadastroConteudo/formConteudo.php?id_conteudo=" . urlencode($id_conteudo) . "&erros=" . urlencode($error));
        exit();
    }
} else {
    $error = "Requisição inválida para atualização de conteúdo.";
    header("Location: ../../consultas/consultaConteudo/consultaConteudo.php?erros=" . urlencode($error));
    exit();
}
?>